...
import com.jeholppx.bbg.mapper.ScoringResultMapper;
import com.jeholppx.bbg.model.dto.scoringResult.ScoringResultQueryRequest;
...
import com.jeholppx.bbg.model.vo.ScoringResultVO;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            ...
            ThrowUtils.throwIf(!JSONUtil.isTypeJSON(resultProp), ErrorCode.PARAMS_ERROR, "结果属性格式不合法");
            ThrowUtils.throwIf(resultScoreRange == null || resultScoreRange < 0, ErrorCode.PARAMS_ERROR, "结果分数范围不合法");
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "应用 id 不合法");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 128, ErrorCode.PARAMS_ERROR, "结果名不能超过128");
        }
